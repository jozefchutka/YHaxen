...
class Main
{
	...
	private function new()
	{
		...
		try
		{
			...
			execute(command);
			System.print("");
			System.print("YHAXEN BUILD SUCCESSFUL");
			Sys.exit(0);
		}
		catch(error:Error)
		{
			System.print("");
			System.print("YHAXEN BUILD FAILED");
	...
	{
		if(Std.is(command, AbstractLifecycleCommand))
			System.print("# config file: " + cast(command, AbstractLifecycleCommand).configFile);
	...
	private function printHelp():Void
	{
		...
		for(item in commands)
			System.print(item.key + " : " + item.info);
	}
}