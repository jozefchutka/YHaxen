...
- neko 2.0.0 does not pass [command line arguments properly escaped into Sys.command](https://groups.google.com/forum/?hl=en#!topic/haxelang/LsKynTHCPXg), so passing -Da=b=c into ant would fail. Current solution is to include white space character (that somehow enforces proper escaping) and remove it on ant side.